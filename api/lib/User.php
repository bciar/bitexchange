...
class User {
	...
	public static function getInfo($session_id=false) {
		...
		$result = db_query_array('SELECT site_users.id,site_users.user,site_users.first_name,site_users.last_name,site_users.country,site_users.email, site_users.default_currency, site_users.default_c_currency, site_users.chat_handle FROM sessions LEFT JOIN site_users ON (sessions.user_id = site_users.id) WHERE sessions.session_id = '.$session_id);
		return $result[0];
	}
